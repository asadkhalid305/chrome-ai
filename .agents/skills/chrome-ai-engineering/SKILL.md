...
4. Keep the native Chrome call visible in the feature's adapter, and the operation call plus its options visible in the feature hook. Do not route core examples through a generic AI SDK or a silent fallback, and do not move an API's own calls into a shared hook.
...
7. Own reusable native sessions through `src/chrome-ai/use-chrome-ai-session.ts`, which holds the session ref, abort controller, capability and download state, and destruction on replacement or cleanup. Avoid a global singleton and avoid reimplementing that plumbing per feature.
